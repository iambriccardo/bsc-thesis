 ...
 * 
 */
public class ModulePlacementPSO0202 extends ModulePlacement {
...
//---------------------------------------------------------------------------------------------------------------------------
    ...
    public ModulePlacementPSO0202(List<FogDevice> fogDevices,
                                  List<Sensor> sensors,
		...
		try {
			...
			bwTIME.write(String.valueOf(this.timeUsed)+"\n");
	...
	protected void Placement() {
		...
		for (int i = 0; i < m_fog_devices ; i++) {
			...
			for	 (int j = 0; j < n_modules; j++) {
				...
				//arr[j] =  (double) 200.00;
				arr[j] = (double) this.getFogDevices().get(i).getHost().getTotalMips() / this.getApplication().getModules().get(j).getMips()*ran.nextInt();
				ram[j] = (double) this.getFogDevices().get(i).getHost().getRam();
		...
		for (int  i = 0; i < this.getFogDevices().size(); i++) {
			...
			for (int j = 0; j < modules.size(); j++) {
				...
				if(binaryValue == 1) {
					...
					//currentModuleMap.put(dev,getApplication().getModuleByName())
				}
			}
